...
def get_rounds(number):
    list_1 = [number, number + 1, number + 2]
    return list_1
...
def approx_average_is_average(hand):
    aprox_average_1 = (hand[0] + hand[-1]) / 2
    aprox_average_2 = hand[len(hand)//2] 
    return card_average(hand) == aprox_average_1 or card_average(hand) == aprox_average_2
...
def average_even_is_average_odd(hand):
    avg_even = sum(hand[0::2])/len(hand[0::2])
